import json
from datetime import datetime
import sqlite3

class RDR:

    @staticmethod
    def getbill(roomid):
        """
        获取指定房间的账单
        """
        connection = None
        cursor = None
        try:
            connection = sqlite3.connect("soft_air.db")
            cursor = connection.cursor()
            # 检查表是否存在
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='clients';")
            result = cursor.fetchone()
            if not result:
                return {"bill": 0, "error": "clients表不存在"}
            # 首先从用户表查询该房间最近的入住记录
            user_query = """
                SELECT 用户名, 用户ID, 入住时间, 结账时间, 总费用   
                FROM clients
                WHERE 房间号 = ?
                ORDER BY 入住时间 DESC
                LIMIT 1
            """
            cursor.execute(user_query, (roomid,))
            user = cursor.fetchone()
            print(f"查询到的用户记录: {user}")
            if not user:
                return {"bill": 0, "error": "未找到用户记录"}

            # 使用数字索引访问元组
            user_id = user[1]       # 用户ID是第2列 (索引1)
            room_id = roomid
            check_in_time = user[2] # 入住时间是第3列 (索引2)
            check_out_time = user[3] # 结账时间是第4列 (索引3)
            print(f"用户ID: {user_id}, 房间号: {room_id}, 入住时间: {check_in_time}, 结账时间: {check_out_time}")
            # 查询该房间在入住期间的所有详单记录
            rdr_query = """
                SELECT SUM(费用) as 总费用
                FROM rdr
                WHERE 房间号 = ? 
                AND 开始时间 >= ? 
                AND 结束时间 <= ?
            """
            cursor.execute(rdr_query, (room_id, check_in_time, check_out_time))
            rdr_result = cursor.fetchone()
            # 确定总费用
            if rdr_result and rdr_result[0] is not None:  # 使用索引0访问SUM结果
                total_cost = float(rdr_result[0])
            else:
                # 如果在详单中没有找到记录，使用用户表中的账单金额
                total_cost = float(user[4]) if user[4] is not None else 0.0  # 总费用是第5列 (索引4)
            # 构建返回字符串
            return {"bill": total_cost}
            
        except sqlite3.Error as e:
            return {"bill": 0, "error": f"数据库错误: {str(e)}"}
        except Exception as e:
            return {"bill": 0, "error": f"处理错误: {str(e)}"}
        finally:
            if cursor:
                cursor.close()
            if connection:
                connection.close()

    @staticmethod
    def getrdr(roomid, type, usrid=None, start_time=None, end_time=None):
        """
        获取指定房间或用户的详单
        
        Args:
            roomid: 房间号
            type: 查询类型，usr 或 room
            usrid: 用户ID（仅在type为usr时使用）
            start_time: 查询开始时间（仅在type为room时使用）
            end_time: 查询结束时间（仅在type为room时使用）
            
        Returns:
            JSON格式的详单
        """
        connection = None
        cursor = None
        try:
            connection = sqlite3.connect("soft_air.db")
            cursor = connection.cursor()

            # 检查表是否存在
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='rdr';")
            result = cursor.fetchone()
            if not result:
                return {"uselist": [], "error": "rdr表不存在"}

            if type == 'usr':
                # 查询该用户最近一次的住宿时间段所包含的所有详单
                user_query = """
                    SELECT 入住时间, 结账时间
                    FROM clients
                    WHERE 用户ID = ?
                    ORDER BY 入住时间 DESC
                    LIMIT 1
                """
                cursor.execute(user_query, (usrid,))
                user = cursor.fetchone()
                
                if not user:
                    return {"uselist": [], "error": "未找到用户记录"}

                check_in_time = user[0]
                check_out_time = user[1]

                rdr_query = """
                    SELECT 房间号, 用户ID, 风速, 开始时间, 结束时间, 费用
                    FROM rdr
                    WHERE 用户ID = ?
                    AND 开始时间 >= ?
                    AND 结束时间 <= ?
                """
                cursor.execute(rdr_query, (usrid, check_in_time, check_out_time))
                rdr_results = cursor.fetchall()
                
                if not rdr_results:
                    return {"uselist": [], "error": "未找到详单记录"}

                # 处理日期时间字段以便JSON序列化
                result_list = []
                for record in rdr_results:
                    record_dict = dict(record)
                    for key, value in record_dict.items():
                        if isinstance(value, datetime):
                            record_dict[key] = value.strftime('%Y-%m-%d %H:%M:%S')
                    result_list.append(record_dict)

                return {"uselist": result_list}
                
            elif type == 'room':
                if not start_time or not end_time:
                    return {"uselist": [], "error": "查询房间详单需要提供开始和结束时间"}

                rdr_query = """
                    SELECT 房间号, 用户ID, 风速, 开始时间, 结束时间, 费用
                    FROM rdr
                    WHERE 房间号 = ?
                    AND 开始时间 >= ?
                    AND 结束时间 <= ?
                """
                cursor.execute(rdr_query, (roomid, start_time, end_time))
                rdr_results = cursor.fetchall()
                
                if not rdr_results:
                    return {"uselist": [], "error": "未找到详单记录"}

                # 处理日期时间字段以便JSON序列化
                result_list = []
                for record in rdr_results:
                    record_dict = dict(record)
                    for key, value in record_dict.items():
                        if isinstance(value, datetime):
                            record_dict[key] = value.strftime('%Y-%m-%d %H:%M:%S')
                    result_list.append(record_dict)

                return {"uselist": result_list}

            else:
                return {"uselist": [], "error": "无效的查询类型，仅支持'usr'或'room'"}

        except sqlite3.Error as e:
            return {"uselist": [], "error": f"数据库错误: {str(e)}"}
        except Exception as e:
            return {"uselist": [], "error": f"处理错误: {str(e)}"}
        finally:
            if cursor:
                cursor.close()
            if connection:
                connection.close()
                
    @staticmethod
    def get_report(start_time, end_time):
        """
        获取指定时间段内的报表
        
        Args:
            start_time: 查询开始时间
            end_time: 查询结束时间
            
        Returns:
            报表内容
        """
        connection = None
        cursor = None
        try:
            connection = sqlite3.connect("soft_air.db")
            cursor = connection.cursor()

            # 检查表是否存在
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='rdr';")
            result = cursor.fetchone()
            if not result:
                return {"content": "rdr表不存在"}

            report_query = """
                SELECT 房间号, 用户ID, SUM(费用) as 总费用
                FROM rdr
                WHERE 开始时间 >= ? AND 结束时间 <= ?
                GROUP BY 房间号, 用户ID
            """
            cursor.execute(report_query, (start_time, end_time))
            report_results = cursor.fetchall()
            
            if not report_results:
                return {"content": "未找到报表记录"}

            # 处理日期时间字段以便JSON序列化
            result_list = []
            for record in report_results:
                record_dict = dict(record)
                for key, value in record_dict.items():
                    if isinstance(value, datetime):
                        record_dict[key] = value.strftime('%Y-%m-%d %H:%M:%S')
                result_list.append(record_dict)
            
            # 计算所有房间的总费用
            total_cost = sum(float(record['总费用']) for record in result_list)
            
            # 格式化报表内容为一个字符串
            formatted_report = f"查询时间段: {start_time} 至 {end_time}\n\n"
            formatted_report += "房间号\t用户ID\t总费用\n"
            formatted_report += "-------------------------------\n"
            
            for record in result_list:
                formatted_report += f"{record['房间号']}\t{record['用户ID']}\t¥{record['总费用']:.2f}\n"
            
            formatted_report += "===============================\n"
            formatted_report += f"总计: ¥{total_cost:.2f}"
            
            # 返回格式化的报表内容
            return {"content": formatted_report}

        except sqlite3.Error as e:
            return {"content": f"数据库错误: {str(e)}"}
        except Exception as e:
            return {"content": f"处理错误: {str(e)}"}
        finally:
            if cursor:
                cursor.close()
            if connection:
                connection.close()
                
    @staticmethod
    def get_scheduling_info():
        """
        获取当前调度信息
        
        Returns:
            当前调度信息
        """
        connection = None
        cursor = None
        try:
            connection = sqlite3.connect("soft_air.db")
            cursor = connection.cursor()

            # 检查表是否存在
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='ac_status';")
            result = cursor.fetchone()
            if not result:
                return {"serving_queue": [], "waiting_queue": []}

            scheduling_query = """
                SELECT 房间号, 空调状态
                FROM ac_status
                WHERE 结束时间 IS NULL
                ORDER BY 开始时间 DESC
            """
            cursor.execute(scheduling_query)
            scheduling_results = cursor.fetchall()
            
            if not scheduling_results:
                return {"serving_queue": [], "waiting_queue": []}

            # 将房间分到服务队列和等待队列
            serving_queue = []
            waiting_queue = []
            
            for record in scheduling_results:
                room_id = record[0]
                status = record[1]

                if status == 'on':
                    serving_queue.append(room_id)
                elif status == 'waiting':
                    waiting_queue.append(room_id)
            
            # 返回指定格式的调度信息
            return {
                "serving_queue": serving_queue,
                "waiting_queue": waiting_queue
            }

        except sqlite3.Error as e:
            return {
                "serving_queue": [],
                "waiting_queue": [],
                "error": f"数据库错误: {str(e)}"
            }
        except Exception as e:
            return {
                "serving_queue": [],
                "waiting_queue": [],
                "error": f"处理错误: {str(e)}"
            }
        finally:
            if cursor:
                cursor.close()
            if connection:
                connection.close()