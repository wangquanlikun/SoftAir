...
class CentralConditioner:
    ...
    @staticmethod
    def get_last_record(number):  # 查询最后一次使用时风速: high, medium, low,后改为0 1
        connection = sqlite3.connect("soft_air.db")
        cursor = connection.cursor()
    ...
    def create_record(number, v, cur_time, now_temperature, target_temperature, mode):
        ...
        create_table_query = f"""
            ...
            """
        insert_query = f"""
                INSERT INTO rdr (房间号, 风速, 当前温度, 目标温度, 冷热模式, 开始时间, 结束时间, 费用) VALUES (?, ?, ?, ?, ?, ?, ?, ?);
                """
        cursor.execute(create_table_query)
        cursor.execute(insert_query, (number, v, now_temperature, target_temperature, mode, cur_time, None, 0))
        connection.commit()
        cursor.close()
        connection.close()
    ...
    def set_speed_request(self, room_id, fan_speed, now_temperature, target_temperature, mode):
        ...
        self.scheduler.create_status()
        result = self.scheduler.update_request(room_id, fan_speed, now_temperature, target_temperature, mode)
        ...
        if result["state"] == "on":
            ...
            if last:
                ...
                bill = round((current_time - datetime.strptime(last[6], "%Y-%m-%d %H:%M:%S.%f")).total_seconds() / 60 / factor, 2)
                self.update_record(last, current_time, bill)
                
            # 创建新记录
            self.create_record(room_id, fan_speed, current_time, now_temperature, target_temperature, mode)
        ...
        result["state"] = "on" if result["state"] else "off"
    ...
    def delete_server(self, room_id):  # 供PowerOff调用
        ...
        if result:
            if result[2] == 'high':
                ...
                factor = 2
            bill = round((current_time - datetime.strptime(result[6], "%Y-%m-%d %H:%M:%S.%f")).total_seconds() / 60 / factor, 2)
